...
from models import get_db, Document, DocumentStatus, create_tables
...
def test_complete_pipeline():
    ...
    pipeline_result = {
        ...
        'nlp': {
            'success': True,
            'fields_extracted': len([k for k, v in nlp_result.items() if k != 'error' and v])
        },
        'summary': {
            'success': True,
    ...
    all_successful = (
        pipeline_result['ocr']['success'] and
        pipeline_result['nlp']['success'] and
        pipeline_result['summary']['success']
    )

...
def generate_lease_summary(
    extracted_text: str, extracted_data: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
...
def _generate_summary_with_comprehend(
    extracted_text: str, extracted_data: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        key_phrases = [
            phrase["Text"] for phrase in key_phrases_response["KeyPhrases"][:10]
        ]

        summary = _create_structured_summary_with_comprehend(
            extracted_text, extracted_data, key_phrases
        )
...
def _generate_summary_locally(
    ...
    try:
        ...
        if extracted_data and isinstance(extracted_data, dict):
            ...
            if (
                isinstance(parties, dict)
                and parties.get("landlord")
                and parties.get("tenant")
            ):
                summary_parts.append(
                    f"This lease agreement is between {parties['landlord']} (Landlord) and {parties['tenant']} (Tenant)."
                )

            dates = extracted_data.get("dates", {})
            if (
                isinstance(dates, dict)
                and dates.get("effectiveDate")
                and dates.get("expirationDate")
            ):
                summary_parts.append(
                    f"The lease term runs from {dates['effectiveDate']} to {dates['expirationDate']}."
                )

            rent = extracted_data.get("rent", {})
            if isinstance(rent, dict) and rent.get("baseRent"):
                rent_text = f"The base rent is {rent['baseRent']}"
                if rent.get("escalationClauses") and isinstance(
                    rent.get("escalationClauses"), list
                ):
                    escalation = (
                        rent["escalationClauses"][0]
                        if rent["escalationClauses"]
                        else ""
                    )
                    if escalation:
        ...
        return {"summary": summary, "method": "local_extraction", "error": None}
...
def _create_structured_summary_with_comprehend(
    extracted_text: str, extracted_data: Optional[Dict[str, Any]], key_phrases: list
) -> str:
    """
    ...
    if extracted_data and isinstance(extracted_data, dict):
        ...
        if isinstance(rent, dict) and rent.get("baseRent"):
            ...
            if rent.get("escalationClauses") and isinstance(
                ...
                escalation = (
                    rent["escalationClauses"][0] if rent["escalationClauses"] else ""
                )
                if escalation:
                    rent_text += f" with {escalation}"
            rent_text += "."
    ...
    if key_phrases and len(summary_parts) < 3:
        ...
        if relevant_phrases:
            summary_parts.append(
                f"Key terms include: {', '.join(relevant_phrases[:3])}."
            )
...
def _extract_summary_from_text(text: str) -> list:
    ...
    if len(dates_found) >= 2:
        summary_parts.append(
            f"The lease term is from {dates_found[0]} to {dates_found[1]}."
        )

    rent_patterns = [r"\$[\d,]+\.?\d*", r"(\d+) dollars?"]
    ...
    if not summary_parts:
        summary_parts.append(
            "This document contains lease agreement terms and conditions."
        )
...
def validate_summary(summary: str) -> Dict[str, Any]:
    ...
    """
    validation_result = {"is_valid": True, "quality_score": 0.0, "issues": []}

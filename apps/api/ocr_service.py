...
try:
    ...
    USE_TESSERACT = False
    logger.info(
        f"Tesseract OCR libraries not found ({e}), image-based PDF processing disabled"
    )
...
def extract_text_from_document(file_path: str, mime_type: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"text": None, "pages": [], "error": str(e)}
...
def _extract_from_pdf_pdfplumber(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if USE_TESSERACT:
            ...
            raise ImportError(
                "Neither PyPDF2 nor pdfplumber is available for PDF text extraction"
            )
    except Exception as e:
...
def validate_extracted_text(text: str) -> bool:
    ...
    if text_length < 50:
        logger.warning(
            f"Text validation failed: text too short ({text_length} characters, minimum 50)"
        )
    ...
    if alphanumeric_ratio < 0.3:  # At least 30% alphanumeric
        logger.warning(
            f"Text validation failed: low alphanumeric ratio ({alphanumeric_ratio:.2%}, minimum 30%)"
        )
        logger.debug(f"Extracted text preview: {repr(text[:200])}")
        return False

    logger.info(
        f"Text validation passed: {text_length} characters, {alphanumeric_ratio:.2%} alphanumeric"
    )
    return True
...
def _extract_from_pdf_with_ocr(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"OCR completed: extracted {len(full_text)} total characters from {len(images)} pages"
        )
